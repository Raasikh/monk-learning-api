...
# ---------------------------------------------------------------------------
# The dry run asks the database.
# ---------------------------------------------------------------------------

PROBE_SLUG = "zzz-constraint-probe--do-not-use"

# One spoiled row per constraint. The override is a DICT, not a single column,
# and that is load-bearing: a negative that trips two constraints at once
# cannot tell you which one caught it. `licence='CC-BY-NC-4.0'` alone is
# exactly that mistake — it violates the licence enum AND
# anchor_book_required, because a non-generated licence must name an anchor,
# so the probe reported the licence enum as healthy while it was dropped. The
# accompanying `anchor_book` keeps the row legal in every respect but the one
# under test. Each entry must isolate its own constraint; the per-constraint
# ablation in the tests is what proves it still does.
PROBE_NEGATIVES = [
    ("concept_assets_manifest_status", {"manifest_status": "approved"}),
    ("concept_assets_licence_enum",
     {"licence": "CC-BY-NC-4.0", "anchor_book": "placeholder for the probe"}),
    ("concept_assets_source_url_is_url", {"source_url": "generated by gemini"}),
    # The other half of 0038: a generated row may not CLAIM an anchor it never
    # had. Spoiled on its own, with the licence left alone.
    ("concept_assets_anchor_book_required", {"anchor_book": "NCERT Biology XI"}),
    ("concept_assets_sub_index_range", {"sub_index": 26}),
    ("concept_assets_text_check_enum", {"text_check": "looks-fine"}),
    ("concept_assets_arrived_labelled_enum", {"arrived_labelled": "maybe"}),
]


def probe_constraints(supabase, row: Dict[str, object]) -> List[str]:
    """Put one row through every CHECK, then take it back out.

    WHY THIS RUNS IN THE DRY RUN. --execute uploaded 105 masters and 105
    renditions and then failed all 105 database writes on
    `manifest_status = 'approved'`, a constraint the dry run had no way to
    consult. "ready" meant "the manifest and the files are good"; it did not
    mean "the database would take this". Now it means both.

    THE PROBE ROW IS A REAL ROW, not a synthetic constant, and that is the
    whole design. A hand-written probe carrying manifest_status='approved'
    would have passed happily while every real row failed — the check would
    have been testing itself rather than the run. This copies the first ready
    row and changes only the three columns that must be unique, so every value
    the run is about to write is a value the database has now accepted.

    BOTH DIRECTIONS. The positive insert proves the row is admissible. The
    negatives prove the constraints are LIVE — without them a database with
    the CHECKs accidentally dropped would sail through the positive half and
    report the strongest possible green. That is the same guardrail-liveness
    ...
    def cleanup():
        try:
            supabase.table(TABLE).delete().eq("asset_slug", PROBE_SLUG).execute()
        except Exception as err:                      # pragma: no cover
            problems.append(
                f"could not delete the probe row {PROBE_SLUG!r}: {err}\n"
                f"      Delete it by hand — it is not a figure and nothing "
                f"should ever resolve to it.")

    cleanup()                       # a probe left behind by a killed run
    try:
        supabase.table(TABLE).insert(probe).execute()
    except Exception as err:
        cleanup()
        return [f"the database REFUSED a row this run calls ready:\n"
                f"      {err}\n"
                f"      Every ready row has this shape. --execute would upload "
                f"first and then fail this way on all of them."]

    for constraint, overrides in PROBE_NEGATIVES:
        spoiled = dict(probe)
        ...
        try:
            supabase.table(TABLE).delete().eq(
                "asset_slug", f"{PROBE_SLUG}-neg").execute()
        except Exception:
            pass
        spoilt = ", ".join(f"{k}={v!r}" for k, v in overrides.items())
        problems.append(
            f"{constraint} is NOT ENFORCED: the database accepted {spoilt}.\n"
            f"      A migration was not applied, or was applied and then "
            f"dropped. Everything this command checks about those columns is "
            f"advisory until it is back.")

    # The unique ordinal is a different shape — it needs a SECOND row rather
    ...
    try:
        ...
        try:
            supabase.table(TABLE).delete().eq(
                "asset_slug", f"{PROBE_SLUG}-twin").execute()
        except Exception:
            pass
        problems.append(
            "concept_assets_set_ordinal_idx is NOT ENFORCED: two rows shared "
            "(concept_slug, sub_index).\n"
            "      Slot 3 resolves a set by that pair. A duplicate ordinal "
            "means figure a is whichever row the planner reaches first.")
...
def cmd_ingest(args) -> int:
    ...
    if dry:
        ...
        if ready:
            ...
            if probe_bad:
                print("\nCONSTRAINT PROBE FAILED")
                for m in probe_bad:
                    print(f"  {m}")
            else:
                print(f"\nconstraint probe OK — the database accepted a copy "
                      f"of {ready[0].slug} and refused all "
                      f"{len(PROBE_NEGATIVES) + 1} spoiled variants.")
        print("\nDRY RUN — nothing uploaded, nothing written. "
              "Re-run with --execute.")
        return 2 if (refused or probe_bad) else 0
