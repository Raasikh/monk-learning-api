...
def main() -> int:
    ...
    for chapter, rows in sorted(per.items()):
        n_rows = [r for r in rows if r["verdict"] == "n"]
        y, n = len(rows) - len(n_rows), len(n_rows)
        pct = 100.0 * y / len(rows) if rows else 0.0
        need = 0
        while rows and 100.0 * (y + need) / len(rows) < 85.0 and need <= n:
            need += 1
        index.append((chapter, len(rows), y, n, pct, need))
        if not n_rows:
            continue
        ...
        d.text((PAD, PAD + 42),
               f"{n} rows proposed n of {len(rows)} — {pct:.1f}%. "
    ...
    d.text((PAD, PAD + 44),
           f"{tot_n} proposed-n rows across {len(index)} chapters. "
    ...
    for lbl, x in (("rows", 760), ("y", 850), ("n", 920), ("%", 1010), ("to 85", 1120)):
